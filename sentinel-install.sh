...
if ! jq --help 2&> /dev/null; then
  echo "aborting - jq not installed and required"
  exit 1
fi
...
getLatest() {
  ...
  case "${nettool}" in
    ...
    wget)
      ...
      ;;
    curl)
      LATEST_ARR=($(curl -s https://releases.hashicorp.com/index.json 2>/dev/null | jq -r '.sentinel.versions[].version' | sort -t. -k 1,1nr -k 2,2nr -k 3,3nr))
      ;;
  esac
...
[[ $PROC =~ arm ]] && PROC="arm"  # sentinel downloads use "arm" not full arm type

# CREATE FILENAME AND URL FROM GATHERED PARAMETERS
FILENAME="sentinel_${VERSION}_${OS}_${PROC}.zip"
LINK="https://releases.hashicorp.com/sentinel/${VERSION}/${FILENAME}"
SHALINK="https://releases.hashicorp.com/sentinel/${VERSION}/sentinel_${VERSION}_SHA256SUMS"
...
if [[ ! "$cwdInstall" ]]; then
  ...
  rm -rf "${UTILTMPDIR}"
  [[ ! "$STREAMLINED" ]] && echo
  echo "Sentinel Version ${VERSION} installed to ${BINDIR}"
else
  echo "Sentinel Version ${VERSION} downloaded"
fi
